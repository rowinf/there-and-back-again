...
function App() {
  ...
  let onCentroidsLoaded = useCallback((json) => {
    sa2NameLookup.current = json.features.reduce((acc, centroid) => {
      acc[centroid.properties["SA22018_V1_00"]] =
        centroid.properties["SA22018_V1_NAME"];
      return acc;
    }, {});
  ...
  let features = useMemo(() => {
    ...
    let { features } =
      filters.dataset === "e" ? educationGraph.current : commuterGraph.current;
    let edges = features
      .filter((feat) => {
        return (
          feat.properties.sa22018_v1 === Number(activeFeatureId) ||
          feat.properties[sa2Attr] === Number(activeFeatureId)
        );
      })
      .map((edge) => ({
  ...
  let activeFeature = useMemo(() => {
    let outtotaler = (attr) => (collection) =>
      collection.reduce((acc, edge) => {
        if (edge[attr] === -999) {
          return acc;
        }
        if (
          edge[sa2Attr] !== Number(activeFeatureId) &&
          edge.sa22018_v1 === Number(activeFeatureId)
        ) {
          return acc + edge[attr];
        }
        return acc;
      }, 0);
    let intotaler = (attr) => (collection) =>
      collection.reduce((acc, edge) => {
        if (edge[attr] === -999) {
          return acc;
        }
    ...
    let withintotaler = (attr) => (collection) =>
      collection.reduce((acc, edge) => {
        if (edge[attr] === -999) {
          return acc;
        }
  ...
  useEffect(() => {
    ...
    return () => {
      if (
        ...
        mapRef.current.getLayer(`${activeFeatureId}-fill`)
      ) {
  ...
  return (
    <div className="App bg-gray-700 text-white">
      <Transition
        ...
        timeout={duration}
        onExited={() =>
          setTimeout(() => mapRef.current.resize(), duration * 1.1)
        }
        onEntered={() =>
          setTimeout(() => mapRef.current.resize(), duration * 1.1)
        }
        ...
        {(state) => (
          <>
              ...
              filters.dataset === "w" ? (
                <>
                  <Layer
                    ...
                    paint={{ "line-color": "#f28cb1", "line-offset": -2 }}
                    filter={["==", "sa22018_v1", Number(activeFeatureId)]}
                  />
                  <Layer
                    id="outbound-label"
                    type="symbol"
                    ...
                    layout={{
                      ...
                      "text-field": [
                        "get",
                        ["to-string", ["get", "sa2_code_w"]],
                        ["literal", sa2NameLookup.current],
                      ],
                    }}
                    paint={{ "text-color": "rgba(232, 232, 192, 0.9)" }}
                    filter={[
                      "==",
                      "SA22018_V1".toLocaleLowerCase(),
                      Number(activeFeatureId),
                    ]}
                  />
                </>
              ) : null}
              ...
              filters.dataset === "w" ? (
                <>
                  <Layer
                    ...
                    sourceId="graph"
                    layout={{ "line-join": "round", "line-cap": "round" }}
                    paint={{ "line-color": "#8ab9f2", "line-offset": 4 }}
            ...
            <div className="absolute left-0 right-0 bottom-0 flex">
              <div className="w-2/3">
                {commuterGraphLoaded && educationGraphLoaded ? (
                  <Overview
                    data={
                      filters.dataset === "w"
                        ? commuterGraph.current.features
                        : educationGraph.current.features
                    }
                    featureMap={
                      filters.dataset === "w"
                        ? workFeatureMap
                    ...
                    sa2Attr={
                      filters.dataset === "w" ? "sa2_code_w" : "sa2_code_e"
                    }
                    colors={colors}
                  />
                ) : null}
              ...
              <div
                style={{ height: 300 }}
                className="w-1/3 flex flex-col flex-grow-0 flex-initial justify-end"
              >
                    ...
                    ? filtered.map((f) => {
                        ...
                        return (
                          <div key={f} className="flex items-baseline">
                            ...
                            <button
                              className="mx-2 px-2 border border-gray-100 rounded font-semibold"
                              onClick={() => {
                ...
                <div className="flex flex-col flex-1 justify-end p-1">
                  {!filtered.length && !resultsTouched ? (
                    <div
                      className="flex flex-col justify-end items-start flex-1"
                      style={{ height: "70%", width: "70%" }}
                    >
                      <button
                        className="border-gray-100 border border-1 p-1"
                        onClick={() => setResultsTouched(true)}
                      >
                        Dismiss tip
                      </button>
                      <img
                        src={help1}
                        alt="Select a range in the series chart to the left to filter regions here"
                        height="250"
                        width="350"
                      />
                    </div>
                  ...
                  <div className="flex justify-between w-full">
                    ...
                    <div className="text-right">
                      <a href="https://github.com/rowinf/there-and-back-again/">
                        View Github
                        <svg
                          className="icon"
            ...
            <FetchJson
              url="https://gist.githubusercontent.com/rowinf/0dc4187273028359193dd68cb8299209/raw/65f59fde8c750f512bf2bf1a1587debe90c4c827/sa2-centroids.geojson"
              onSuccess={onCentroidsLoaded}
            />
            <FetchJson
              url="https://gist.githubusercontent.com/rowinf/f1a3e8489da942f3c1ba1e8e7ef0d323/raw/c541c3fcfcce9b8b4d96dc4cf1d79bc9e1569345/commuter_graph.geojson"
              onSuccess={onCommuterGraphLoaded}
            />
            <FetchJson
              url="https://gist.githubusercontent.com/rowinf/ee0b4529615f74140bacd5954d5a0016/raw/8c85afd547b8652737fcf32f64b13075a0945f6d/education_graph.geojson"
              onSuccess={onEducationGraphLoaded}
            />
            <div
              className="transition ease-in-out duration-300 overflow-y-auto"
              style={{ ...defaultStyle, ...transitionDrawerStyles[state] }}
            >
              {activeFeature && (
                <Drawer
              ...
              <button
                className="absolute top-0 right-0 font-bold py-2 px-4 rounded inline-flex items-center"
